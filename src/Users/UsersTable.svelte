...
users, { rowsPerPage: rowsPerPageData, css: true, i18n: {
    search: 'Buscar...',
    show: 'Mostrar',
    entries: 'clientes',
    filter: 'Filtrar',
...
const deleteUser = async (id) => {
  ...
  if (response.ok) {
    // Remover el usuario eliminado de la lista
...
Swal.fire(
  ...
  const editUser = (id) => {
    broteNavigate(`/edit-user/${id}`); // Ajusta para usar navigate
  };

  const viewUser = (id) => {
    broteNavigate(`/view-user/${id}`); // Ajusta para usar navigate
  };
</script>

<div class="container mt-4">
  <h1 class="mb-4">Ver Usuarios <small class="text-muted">  <a href="javascript:void(0);" on:click={(event) =>  broteNavigate('/create-user')} class="addBtn btn-success btn btn-sm mb-3 inline ">Agregar</a>
  </small></h1>
